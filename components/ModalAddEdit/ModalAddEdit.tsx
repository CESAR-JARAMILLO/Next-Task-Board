...
const ModalAddEdit = ({selectedModalType} : ModalAddEditProps) => {
  const [modalType, setModalType] = useState(selectedModalType);
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [subtasks, setSubtasks] = useState([{ id: Date.now(), content: '' }]);
  ...
  const addSubtask = () => {
    setSubtasks([...subtasks, { id: Date.now(), content: '' }]);
  };
  ...
  return (
    <Box className={styles.modalAddEdit}>
      <Title className={styles.modalAddEditTitle} order={2}>Add New Task</Title>
      ...
      <Button classNames={{ root: styles.submitButtonRoot }} onClick={handleSubmit}>Create Task</Button>
    </Box>
  );
}
